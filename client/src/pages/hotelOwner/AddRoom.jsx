...
const AddRoom = () => {
  ...
  const onSubmitHandler = async (e) => {
    ...
    if(!inputs.roomType || !inputs.pricePerNigth || !inputs.amenities || !Object.values(images).some(image => image)) {
    ...
    try {
      ...
      formData.append('pricePerNight', inputs.pricePerNigth)
        // Converting amenties to array & keeping only anabled amenities
        const amenities = Object.keys(inputs.amenities).filter(key => inputs.amenities[key])
        formData.append('amenitites', JSON.stringify(amenities))
    ...
    <form onSubmit={onSubmitHandler}>
      ...
      <div className='w-full flex max-sm:flex-col sm:gap-4 mt-4'>
        ...
        <div>
          ...
          <input type="number" placeholder='0' className='border border-gray-300 mt-1 rounded p-2 w-24' value={inputs.pricePerNigth} onChange={e=> setInputs({...inputs, pricePerNigth: e.target.value})} />
        </div>
      </div>
